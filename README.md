...
```
lab/runner.py       execute a prompt over a dataset, score each case
lab/scorers.py      pluggable scorers: per-field match, label match
lab/cache.py        SQLite response cache
lab/store.py        runs, case_results, candidates
lab/select.py       score candidates on dev, pick a winner
lab/optimizers/     bootstrap (demos), mutate (rewrite), append (add a rule)
lab/load_emotion.py build the emotion task from the HF datasets API
tasks/<name>/       dataset.json, prompt.txt, config.json, seeds.json
NOTES.md            full experiment log, including the results that failed
```
